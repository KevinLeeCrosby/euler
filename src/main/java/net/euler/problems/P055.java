...
public class P055 {
  ...
  private static boolean isPalindrome(final BigInteger n) {
  ...
  public static void main(String[] args) {
    ...
    for(int number = limit - 1; number >= 100; number--) {
      ...
      if(lychrelCache.contains(n) || nonLychrelCache.contains(n)) {
        continue;
      }
      ...
      for(int iteration = 1; iteration < iterations; iteration++) {
        n = reverseAndAdd(n);
        isNonLychrel = isPalindrome(n) || nonLychrelCache.contains(n);
        isLychrel = lychrelCache.contains(n);
        if(isNonLychrel || isLychrel) {
          break;
        }
